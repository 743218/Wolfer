...
namespace mce {
	...
	struct MaterialPtr {
		static MaterialPtr* createMaterial(HashedString const& materialName) {
			static void* materialCreator = (void*)Memory::getVtableFromSig("48 8B 05 ? ? ? ? 48 8D 55 ? 48 8D 0D ? ? ? ? 48 8B 40 ? FF 15 ? ? ? ? 48 8B F8");
			return Memory::CallVFunc<1, MaterialPtr*, HashedString const&>(materialCreator, materialName);
		}
	};
}
