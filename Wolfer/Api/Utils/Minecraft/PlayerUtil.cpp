...
bool PlayerUtil::tryPlaceBlockSupport(const BlockPos& pos, int proximity, bool airplace, bool packetPlace) {
	...
	std::sort(checkList.begin(), checkList.end(), [&](const BlockPos& start, const BlockPos& end) {
		...
		return posFloat.dist(start.toFloat()) < posFloat.dist(end.toFloat());
		});

	auto it = std::find_if(checkList.begin(), checkList.end(), [](const BlockPos& current) {
...
bool MoveUtil::isMoving() {
	if (g_Data.isKeyDown('W') || g_Data.isKeyDown('A') || g_Data.isKeyDown('S') || g_Data.isKeyDown('D')) return true;
	return false;
}
...
Vec2<float> MoveUtil::getMotion(const float& speed) {
	...
	bool s = g_Data.isKeyDown('S');
	bool d = g_Data.isKeyDown('D');
	if (w) {
...
float MoveUtil::getAdjustedYaw() {
	float yaw = g_Data.getLocalPlayer()->rotation->presentRot.y;
	if (g_Data.isKeyDown('W') && g_Data.isKeyDown('S')) return yaw;
	else if (g_Data.isKeyDown('W') && g_Data.isKeyDown('D') && !g_Data.isKeyDown('A')) yaw += 45.f;
	else if (g_Data.isKeyDown('W') && g_Data.isKeyDown('A') && !g_Data.isKeyDown('D')) yaw -= 45.f;
	else if (g_Data.isKeyDown('S') && g_Data.isKeyDown('D') && !g_Data.isKeyDown('A')) yaw += 135.f;
	else if (g_Data.isKeyDown('S') && g_Data.isKeyDown('A') && !g_Data.isKeyDown('D')) yaw -= 135.f;
	else if (g_Data.isKeyDown('S')) yaw += 180.f;
	else if (g_Data.isKeyDown('D') && !g_Data.isKeyDown('A')) yaw += 90.f;
	else if (g_Data.isKeyDown('A') && !g_Data.isKeyDown('D')) yaw -= 90.f;
...
void MoveUtil::setSpeed(const float& speed) {
	...
	g_Data.getLocalPlayer()->stateVector->velocity.x = motion.x;
	g_Data.getLocalPlayer()->stateVector->velocity.z = motion.y;
}