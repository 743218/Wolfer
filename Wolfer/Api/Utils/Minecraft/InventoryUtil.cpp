...
PlayerInventory* InventoryUtil::getPlayerInventory() {
	return g_Data.getLocalPlayer()->playerInventory;
}
...
uint8_t InventoryUtil::getSelectedSlot() {
	return g_Data.getLocalPlayer()->playerInventory->selectedSlot;
}
...
void InventoryUtil::sendMobEquipment(uint16_t slot) {
	...
	MobEquipmentPacket mep(g_Data.getLocalPlayer()->getRuntimeIDComponent()->runtimeId, InventoryUtil::getItem(slot), slot, slot);
	g_Data.getLocalPlayer()->level->getPacketSender()->send(&mep);
}
...
void InventoryUtil::swapItem(uint8_t from, uint8_t to, bool packet) {
	InventoryTransactionManager* invMgr = g_Data.getLocalPlayer()->getTransactionManager();
	...
	if (!packet) {
		...
		g_Data.getLocalPlayer()->level->getPacketSender()->send(&pk);
	}
}
