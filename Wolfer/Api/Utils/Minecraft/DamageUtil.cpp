...
float DamageUtils::getExplosionDamage(const Vector3<float>& position, Actor* actor, float extrapolation, int ignoredBlockId) {
	...
	if (ignoredBlockId != 0) exposure = g_Data.clientInstance->getRegion()->getBlock((position.x, position.y, position.z))->blockLegacy->blockId == ignoredBlockId ? 1.f : g_Data.clientInstance->getRegion()->getSeenPercent(position, actor->aabbShape->aabb);
	else exposure = g_Data.clientInstance->getRegion()->getSeenPercent(position, actor->aabbShape->aabb);
