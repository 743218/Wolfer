...
public:
    ...
    void onD2DRender() override {
        ...
        Vec2<float> winSize = g_Data.clientInstance->guiData->windowSizeReal;
        Vec2<float> winReal = g_Data.clientInstance->guiData->windowSizeReal;

        float textSize = 1.5f;
        float padX = 2.f;
        float padY = 2.f;
        float textHeight = D2D::getTextHeight("A", textSize);
        Vec2<float> basePos(winSize.x - 2.f, winSize.y - 2.f);

        static std::map<Module*, Vec2<float>> animMap;
        ...
        for (auto& mod : list) {
            ...
            Vec2<float> targetPos(basePos.x - padX - fullWidth, basePos.y - textHeight - yOffset);
            Vec2<float>& animPos = animMap[mod];
            ...
            if (enabled) {
                ...
                Vec2<float> drawPos(animPos.x * (winReal.x / winSize.x), animPos.y * (winReal.y / winSize.y));
                D2D::drawText(drawPos, name, color, textSize);
                D2D::drawText(Vec2<float>(drawPos.x + nameWidth, drawPos.y), ">", WolferColor(150, 150, 150), textSize);

                yOffset += textHeight;
            }
            else {
                Vec2<float> offscreenPos(basePos.x + 20.f, animPos.y);
        ...
        Vec2<float> posLabelPos(padX, winSize.y - textHeight - padY);
        Vec2<float> posLabelPosReal(posLabelPos.x * (winReal.x / winSize.x), posLabelPos.y * (winReal.y / winSize.y));
        ...
        D2D::drawText(Vec2<float>(startXReal, posYReal), " [", gray, textSize);
        startX += D2D::getTextWidth(" [", textSize);
        startXReal = startX * (winReal.x / winSize.x);

        D2D::drawText(Vec2<float>(startXReal, posYReal), xStr, white, textSize);
        startX += xWidth;
        startXReal = startX * (winReal.x / winSize.x);

        D2D::drawText(Vec2<float>(startXReal, posYReal), comma, gray, textSize);
        startX += commaWidth;
        startXReal = startX * (winReal.x / winSize.x);

        D2D::drawText(Vec2<float>(startXReal, posYReal), yStr, white, textSize);
        startX += yWidth;
        startXReal = startX * (winReal.x / winSize.x);
        ...
        D2D::drawText(Vec2<float>(startXReal, posYReal), zStr, white, textSize);
        startX += zWidth;
        startXReal = startX * (winReal.x / winSize.x);
        ...
        Vec2<float> wmPos(winSize.x - wmWidth - padX, padY);
        Vec2<float> wmPosReal(wmPos.x * (winReal.x / winSize.x), wmPos.y * (winReal.y / winSize.y));
