...
bool FriendCommand::execute(const std::vector<std::string>& args) {
	...
	if (args[1] == "remove") {
		...
		if (!trgtName.empty()) {
			if (FriendManager::removeFromList(trgtName)) g_Data.DisplayClientMessage("%sRemoved %s%s%s to list.", MCTF::GREEN, MCTF::YELLOW, trgtName, MCTF::GREEN);
			else g_Data.DisplayClientMessage("%s%s%s was not in your friendslist", MCTF::YELLOW, searchedName.c_str(), MCTF::RED);
			return true;
		}
		else {
			g_Data.DisplayClientMessage("%s%s%s was not in your friendslist", MCTF::YELLOW, searchedName.c_str(), MCTF::RED);
			return true;
		}
	}
	else if (args[1] == "add") {
		...
		for (Actor* actor : g_Data.getLocalPlayer()->level->getRuntimeActorList()) {
		...
		if (target != nullptr) {
			...
			if (FriendManager::addToList(trgtName)) g_Data.DisplayClientMessage("%sAdded %s%s%s to list.", MCTF::GREEN, MCTF::YELLOW, trgtName, MCTF::GREEN);
			else g_Data.DisplayClientMessage("%s%s%s is already in your friendslist", MCTF::YELLOW, trgtName.c_str(), MCTF::RED);
			return true;
		}
		else {
			g_Data.DisplayClientMessage("%sEntity matching the name of %s%s%s was not found, please make sure they are on your render distance", MCTF::RED, MCTF::YELLOW, searchedName.c_str(), MCTF::RED);
			return true;
		}
	}
	return false;
